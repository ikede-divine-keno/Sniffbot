...
def test_task_result_structure():
    response = jsonrpc_call("message/send", {
        "message": {"role": "user", "parts": [{"kind": "text", "text": "@sniffbot sniff this\n```python\nx=1\n```"}]}
    })
    if response.status_code != 200:
        return
...
def test_code_extraction_inline():
    response = jsonrpc_call("message/send", {
        "message": {"role": "user", "parts": [{"kind": "text", "text": "@sniffbot sniff this `x = 1 + 2`"}]}
    })
    if response.status_code != 200:
...
def test_memory_follow_up():
    ...
    text = result2["status"]["message"]["parts"][0]["text"]
    print(f"Follow-up response text: {text}")

    assert ctx1 == ctx2
    assert any(keyword in text.lower() for keyword in ["welcome", "more"]), f"Expected 'welcome' or 'more' in response: {text}"
    add_points(8, "Memory: follow-up uses same contextId + friendly reply")
...
def test_fix_last_command():
    ...
    resp2 = jsonrpc_call("execute", {
        "messages": [
            ...
            {
                "role": "user",
                "parts": [{"kind": "text", "text": "@sniffbot fix last"}]
            }
        ],
        "contextId": ctx,
        "taskId": str(uuid.uuid4())
...
def test_scheduler_job_exists():
    ...
    # Parse ISO → validate future
    dt = datetime.fromisoformat(next_run.replace("Z", "+00:00"))
    from datetime import timezone  # ← ADD THIS
    assert dt > datetime.now(timezone.utc)  # ← FIX: use timezone.utc
